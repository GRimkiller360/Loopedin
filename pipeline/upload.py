...
def _upload_captions(youtube, video_id, srt_path):
    # A real caption track (2026-08-26), separate from the burned-in TikTok-style
    # captions the video already has pixel-baked in for the watch-muted case -- this
    # is what lets YouTube itself read the spoken content as text (search/suggested
    # matching) and gives a real accessibility track, instead of relying on YouTube's
    # own auto-ASR captions, which reliably mangle this channel's own historical
    # proper nouns. Generated by pipeline/assemble.py's _write_srt() from the exact
    # same real per-beat audio timing the burned-in track uses, so the two can't drift
    # against each other. isDraft=False so it's live immediately, not sitting
    # unpublished waiting for manual approval.
    from googleapiclient.http import MediaFileUpload

    youtube.captions().insert(
        part="snippet",
        body={"snippet": {"videoId": video_id, "language": "en", "name": "", "isDraft": False}},
...
def upload_short(video_path, script, privacy_status="public", captions_path=None):
    from googleapiclient.http import MediaFileUpload
    ...
    # Also best-effort -- same reasoning as the comment/thumbnail steps above: the
    # video already published successfully, so a captions API hiccup must never be
    # reported as an upload failure. captions_path is optional (None if the caller
    # doesn't have one, e.g. an older assemble.py run) so this silently no-ops rather
    # than erroring when there's nothing to upload.
    if captions_path and Path(captions_path).exists():
        try:
            _upload_captions(youtube, video_id, captions_path)
        except Exception as e:
            print(f"warning: caption upload failed (non-fatal): {e}", file=sys.stderr)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--captions", default=None, help="path to an SRT file (pipeline/assemble.py's captions.srt) to upload as a real YouTube caption track; optional, best-effort")
    args = parser.parse_args()

    script_data = json.loads(Path(args.script).read_text(encoding="utf-8"))
    try:
        video_id = upload_short(args.video, script_data, args.privacy, args.captions)
